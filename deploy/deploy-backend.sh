...
cmd_deploy() {
    log_info "🚀 Деплой сервера бэкенда..."
    check_requirements
    source_env
    cmd_pull
    ...
    sleep 10
    cmd_migrate
    log_success "✅ Сервер бэкенда задеплоен!"
}

show_help() {
    cat << EOF
${BLUE}Worker Cabinet — скрипт деплоя сервера бэкенда${NC}

Использование: $0 [COMMAND]

Команды:
  deploy      Загрузка образов, пересоздание контейнеров и миграции
...
Примеры:
  $0 deploy          Полный деплой с миграциями
