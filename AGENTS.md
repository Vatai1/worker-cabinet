...
2. **Schema changes**: Add SQL to `backend/src/db/migrate.js` (monolithic migration)
3. **Date handling**: `formatDate()` from `@/lib/utils` — parses `YYYY-MM-DD` as local date
4. **Environment**: `cp backend/.env.example backend/.env` — configure `DB_*`, `JWT_SECRET`, `S3_*`, `TELEGRAM_BOT_TOKEN`
