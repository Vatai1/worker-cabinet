...
app.use((req, res, next) => {
  ...
  let allowedOrigin = !origin || allowed.includes(origin)
  if (!allowedOrigin) {
    const host = req.headers.host
    if (host) {
      try {
        allowedOrigin = new URL(origin).hostname === host.split(':')[0]
      } catch {
        allowedOrigin = false
      }
    }
  ...
  if (!allowedOrigin) {
    ...
    return res.status(403).json({ error: 'Not allowed by CORS', code: 'CORS_FORBIDDEN' })
  }
