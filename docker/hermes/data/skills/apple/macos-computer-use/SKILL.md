...
- **"Connection refused" on cua-driver.sock** — The daemon process may be
  running but bound to a stale socket. Check `ls -la ~/Library/Caches/cua-driver/cua-driver.sock` — if the mtime is days old, the daemon hung. Fix: `kill <pid> && open /Applications/CuaDriver.app`.
- **AX elements with empty labels and [0,0,0,0] bounds** — Some browsers
  (notably **Comet**) expose AX elements but without any useful text or
  geometry. `mode=som` returns a numbered index but clicking is impossible
  because bounds are zeroed and labels are blank. `mode=vision` may also
  return 0x0 dimensions. In this case the browser is not compatible with
  computer_use — ask the user to use Safari or Chrome instead.
